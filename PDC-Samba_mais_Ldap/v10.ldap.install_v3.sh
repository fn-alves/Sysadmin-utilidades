...
echo ""
echo "    Para Debian Lenny o repositorio sera configurado para"
echo "    aceitar o backports"
...
if [ "`cat /etc/debian_version | cut -c\-\2`" = "5." ];then
   ...
   if [ "`cat /etc/apt/sources.list | grep backports.debian.org`" = "" ];then
...
if [ "$DISTRO" = "LENNY" ];then
	...
	apt-get install -y --force-yes slapd ldap-utils db4.6-util libpam-ldap libnss-ldap nscd libpam-foreground mcrypt libgd-tools phpldapadmin resolvconf apache2-suexec libapache2-mod-php5 php5 php5-cli php5-curl php5-gd php5-imap php5-ldap php5-mcrypt php5-mhash php5-sqlite php5-tidy php5-xmlrpc php-pear mcrypt apache2-doc nfs-kernel-server ssl-cert

	else
	apt-get install -y --force-yes slapd ldap-utils db4.6-util libpam-ldap libnss-ldap nscd libpam-foreground mcrypt libgd-tools samba samba-common samba-doc smbclient smbfs smbldap-tools phpldapadmin resolvconf apache2-suexec libapache2-mod-php5 php5 php5-cli php5-curl php5-gd php5-imap php5-ldap php5-mcrypt php5-mhash php5-sqlite php5-tidy php5-xmlrpc php-pear mcrypt apache2-doc nfs-kernel-server ssl-cert
...
#  do SLAPD para iniciar atraves do      #
#  slapd.conf ao inves do diretorio      #
#  slapd.d                               #
#                                        #
##########################################

if [ "`ps aux | grep openldap | grep \"\-\F \/etc\/ldap\/slapd.d\"`" != "" ];then
   
   /etc/init.d/slapd stop
   sed "s/SLAPD_CONF=\/etc\/ldap\/slapd.d/SLAPD_CONF=\/etc\/ldap\/slapd.conf/g" /etc/init.d/slapd > /tmp/slapd
   mv /tmp/slapd /etc/init.d/slapd
...
if [ "$REMOTO" != "N" ]; then
	...
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "#" >> /mnt/configurador/cliente_ldap.sh
	echo "# Configurar o home dos usuarios" >> /mnt/configurador/cliente_ldap.sh
	echo "#" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"#!/bin/bash\" > /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"LDAPSEARCH=\\\"\`which ldapsearch\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"ECHO=\\\"\`which echo\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"WGET=\\\"\`which wget\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"GREP=\\\"\`which grep\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"MKDIR=\\\"\`which mkdir\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"CAT=\\\"\`which cat\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"CHOWN=\\\"\`which chown\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"CHMOD=\\\"\`which chmod\`\\\"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"for i in \\\`\\\$LDAPSEARCH -h `uname -n` -x -b \\\"ou=Users,dc=`echo \$DOMINIO | tr '[:upper:]' '[:lower:]' | sed \"s/\./,dc=/g\"`\\\" | grep uid= | cut -d\\\" \\\" -f2 | cut -d\\\"=\\\" -f2 | cut -d\\\",\\\" -f1 | grep -v root | grep -v nobody\\\`;do\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"	if [ ! -d `echo $SMBUSR | tr '[:upper:]' '[:lower:]'`/\\\$i ];then\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"	      \\\$MKDIR -p `echo $SMBUSR | tr '[:upper:]' '[:lower:]'`/\\\$i \" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"	      \\\$CHOWN \\\$i.\\\"Domain Users\\\" `echo $SMBUSR | tr '[:upper:]' '[:lower:]'`/\\\$i\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"	fi\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"	\\\$CHOWN \\\$i.\\\"Domain Users\\\" `echo $SMBUSR | tr '[:upper:]' '[:lower:]'`/\\\$i\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"done\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\\\$CHMOD 755 `echo $SMBUSR | tr '[:upper:]' '[:lower:]'`/*\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "echo \"\" >> /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "chmod 751 /etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "/etc/init.d/home.sh" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "update-rc.d -f home.sh start 99 2 ." >> /mnt/configurador/cliente_ldap.sh
	echo "if [ \"\`cat /etc/crontab | grep /etc/init.d/home.sh\`\" = \"\" ];then" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "	echo \"\" >> /etc/crontab" >> /mnt/configurador/cliente_ldap.sh
	echo "	echo \"*/30 *	* * *	root /etc/init.d/home.sh\" >> /etc/crontab" >> /mnt/configurador/cliente_ldap.sh
	echo "	echo \"\" >> /etc/crontab" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
	echo "fi" >> /mnt/configurador/cliente_ldap.sh
	echo "" >> /mnt/configurador/cliente_ldap.sh
